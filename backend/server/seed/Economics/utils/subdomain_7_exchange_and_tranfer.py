# This script adds values for columns present in "new_cols"
# It uses https://datausa.io to get the data
# sample: https://datausa.io/api/data?Origin State=16000US3651000&measure=Millions%20Of%20Dollars,Thousands%20Of%20Tons&drilldowns=Destination State
...
datausa_map = {}
# id exceptions for https://datausa.io
...
def update_row(key):
    ...
    base = f'https://datausa.io/api/data?Origin State={location_id}&measure=Millions%20Of%20Dollars,' \
           f'Thousands%20Of%20Tons&drilldowns=Destination State'
    print(base)
    data = requests.get(base).text
    time.sleep(1)
    try:
        past_trade_dollars, past_trade_tons, future_trade_dollars, future_trade_tons = getData(data, "2020", "2030")
...
def update_500_cities():
    ...
    # new columns
    new_cols = ["past_trade_dollars_2020", "past_trade_tons_2020", "future_trade_dollars_2030",
...
# uses "data" and returns [past_trade_dollars, past_trade_tons, future_trade_dollars, future_trade_tons]
# where past = past_year
# and future = future_year
