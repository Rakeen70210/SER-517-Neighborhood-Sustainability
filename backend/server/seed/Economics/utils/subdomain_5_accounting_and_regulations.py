# This script adds columns present in "account_and_regul" list to output CSV
# It uses eg site: https://www.city-data.com/city/New-York-New-York.html
...
# To get the numeric value from list (li) string of the sections
...
# Page scraping using DOM
# It returns [status, city_details map]
def get_acc_and_reg_data(link):
    print(link)
    # link = "https://www.city-data.com/city/Indianapolis-Indiana.html"
    html_text = requests.get(link).text
    soup = BeautifulSoup(html_text, 'html.parser')
    cityDetails = ["N" for _ in account_and_regul]
...
# It returns a dictionary for getting url extension for base url
# It uses files present in data folder.
...
if __name__ == '__main__':
    ...
    account_and_regul = ["government_finances_expenditure_per_resident_in_2018($)",
    ...
    # exception mappings for https://www.city-data.com/
    exceptionsFor500 = {
    ...
    # might be used in future findClosestMatch
    ...
    with open("../../final500Cities.csv", "r") as readObj:
        ...
        # add new columns
        def addNewColumns():
            headers.extend(account_and_regul)
        ...
        for row in csvReader:
            ...
            if key in cityDataCities:
                status, cityHouseDetails = get_acc_and_reg_data(cityDataCities[key])
                if status:
                    totalPassed += 1
                else:
                    totalFailed += 1
                ...
                status, cityHouseDetails = get_acc_and_reg_data(cityDataCities[exceptionsFor500[key]])
                if status:
                    totalPassed += 1
                else:
                    totalFailed += 1
